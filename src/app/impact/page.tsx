  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      ...
      <p className="mt-3 max-w-2xl text-ink/70">
        Live registry metrics and investment placeholders for UNICEF Venture Fund–style
        reporting. Machine-readable feed:{" "}
        <Link href="/api/kpi" className="font-semibold text-ocean hover:underline">
          GET /api/kpi
        </Link>
        .
      </p>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Countries" value={String(kpi.coverage.countries)} icon={<Globe2 className="h-5 w-5" />} />
        <Stat label="Tiered cities" value={String(kpi.coverage.cities)} icon={<Layers className="h-5 w-5" />} />
        <Stat label="India regions" value={String(kpi.coverage.indiaRegions)} icon={<HeartHandshake className="h-5 w-5" />} />
        <Stat label="Trusted sources" value={String(kpi.coverage.trustedSources)} icon={<BarChart3 className="h-5 w-5" />} />
      </div>
      ...
      <section className="mt-10 grid gap-6 md:grid-cols-2">
        ...
        <Card title="Investment placeholders">
          <ul className="space-y-2 text-sm text-ink/80">
            <li>Seed requested: ${kpi.investment.seedRequested.toLocaleString()} {kpi.investment.currency}</li>
            <li>Pilot sites committed: {kpi.investment.pilotSitesCommitted}</li>
            <li>Guided sessions target: {kpi.investment.guidedSessionsTarget}</li>
            <li>Languages shipped / target: {kpi.investment.languagesShipped} / {kpi.investment.languagesTarget}</li>
          </ul>
